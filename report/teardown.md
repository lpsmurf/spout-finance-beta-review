...
| 🟠 | **Order fulfillment is opaque and keeper-paced** — our $4 GOOG buy's escrow account (`6s9tiW4Rcz…`) shows only the placement tx on-chain, still `pending` ~2h later; the feed exposes no status/ETA and the UI doesn't surface the escrow. Fills are likely market-hours-gated (Alpaca) — defensible, but the app never says that; the order just sits. |
